"""UDP Load Balancer metrics collector for F5XC."""
...
class UdpLoadBalancerCollector:
    ...
    def __init__(self, client: F5XCClient):
        """Initialize UDP load balancer collector."""
        self.client = client
        ...
        self.collection_success = Gauge(
            "f5xc_udp_lb_collection_success",
            "Whether UDP LB metrics collection succeeded (1=success, 0=failure)",
            []
        )

        self.collection_duration = Gauge(
            "f5xc_udp_lb_collection_duration_seconds",
            "Time taken to collect UDP LB metrics",
            []
        )

    def collect_metrics(self) -> None:
        """Collect UDP load balancer metrics for all namespaces."""
        ...
        try:
            ...
            logger.info(
                "UDP LB metrics collection successful",
                duration=collection_duration,
            )

        except F5XCAPIError as e:
            logger.error(
                "Failed to collect UDP LB metrics",
                error=str(e),
                exc_info=True,
            )
    ...
    def _process_response(self, data: Dict[str, Any]) -> None:
        ...
        logger.debug("Processing UDP LB nodes", node_count=len(nodes))
