...
class TestCardinalityUnlimitedTracking:
    """Tests for unlimited tracking when limits are set to 0."""

    def test_unlimited_namespaces(self):
        """Test that setting max_namespaces=0 allows unlimited tracking."""
        tracker = CardinalityTracker(max_namespaces=0)

        # Should be able to track many namespaces without limit
        for i in range(200):
            assert tracker.check_namespace_limit(f"ns{i}", "test") is True
    ...
    def test_unlimited_load_balancers(self):
        ...
        tracker = CardinalityTracker(max_load_balancers_per_namespace=0)

        # Should be able to track many load balancers without limit
        for i in range(100):
            assert tracker.check_load_balancer_limit("ns1", f"lb{i}", "test") is True
    ...
    def test_unlimited_dns_zones(self):
        ...
        tracker = CardinalityTracker(max_dns_zones=0)

        # Should be able to track many DNS zones without limit
        for i in range(200):
    ...
    def test_mixed_limited_and_unlimited(self):
        ...
        tracker = CardinalityTracker(
            ...
            max_load_balancers_per_namespace=2,  # Limited
            max_dns_zones=0,  # Unlimited
        )

        # Namespaces should be unlimited
        for i in range(10):
            assert tracker.check_namespace_limit(f"ns{i}", "test") is True

        # Load balancers should be limited to 2 per namespace
        assert tracker.check_load_balancer_limit("ns1", "lb1", "test") is True
        assert tracker.check_load_balancer_limit("ns1", "lb2", "test") is True
        assert tracker.check_load_balancer_limit("ns1", "lb3", "test") is False

        # DNS zones should be unlimited
        for i in range(10):
            assert tracker.check_dns_zone_limit(f"zone{i}.com", "test") is True
